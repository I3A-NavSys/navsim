...
class UAVcontrol:
    def __init__(self, rigid_prim_view, torch_device, uavs, operator_event, event_stream):
        ...
        self.thrust_coeffs = np.array([kFT_N, kFT_N, kFT_S, kFT_S])
        self.drag_force_coeffs = -np.array([kFDx, kFDy, kFDz])
        self.drag_moment_coeffs = np.array([kMDR_N, kMDR_N, kMDR_S, kMDR_S])
    ...
    def servo_control(self, i, uav_id):
        ...
        lin_vel_error = self.local_lin_vel - command[:3]
        ang_vel_error = self.local_ang_vel[2] - command[3]
        ...
        # Get rotor velocities from the policy
        self.rotors_vel = (self.policy(observations) * self.policy_actions_scale).cpu().detach().numpy()
