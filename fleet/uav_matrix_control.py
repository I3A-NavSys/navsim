...
class UAVcontrol:
    def __init__(self, rigid_prim_view, torch_device, uavs, operator_event, event_stream):
        ...
        # Control state arrays
        self.Hs = np.full((self.n, 4, 1), self.w_hov)
        self.r = np.zeros((self.n, 4, 1))
        self.x = np.zeros((self.n, 8, 1))
        self.y = np.zeros((self.n, 4, 1))
        self.e = np.zeros((self.n, 4, 1))
        self.E = np.zeros((self.n, 4, 1))
        self.u = np.zeros((self.n, 4, 1))
        self.E_max = 150
    ...
    def _batch_servo_control(self):
        """Fully vectorized servo control"""
        ...
        # Vectorized command extraction
        self.r[control_active, 0, 0] = [self.commands[i].velX for i in np.where(control_active)[0]]
        self.r[control_active, 1, 0] = [self.commands[i].velY for i in np.where(control_active)[0]]
        self.r[control_active, 2, 0] = [self.commands[i].velZ for i in np.where(control_active)[0]]
        self.r[control_active, 3, 0] = [self.commands[i].rotZ for i in np.where(control_active)[0]]
        
        # Vectorized state assignment
        self.x[control_active, 0, 0] = self.eulers[control_active, 0]  # roll
        self.x[control_active, 1, 0] = self.eulers[control_active, 1]  # pitch
        self.x[control_active, 2:5, 0] = self.local_ang_vels[control_active]
        self.x[control_active, 5:8, 0] = self.local_lin_vels[control_active]
        
        # Vectorized output assignment
        self.y[control_active, 0, 0] = self.x[control_active, 5, 0]  # velX
        self.y[control_active, 1, 0] = self.x[control_active, 6, 0]  # velY
        self.y[control_active, 2, 0] = self.x[control_active, 7, 0]  # velZ
        self.y[control_active, 3, 0] = self.x[control_active, 4, 0]  # angZ
        
        # Vectorized control computation
        self.e[control_active] = self.y[control_active] - self.r[control_active]
        self.E[control_active] += self.e[control_active] * self.step_size
        self.E = np.clip(self.E, -self.E_max, self.E_max)
        
        # Batch matrix operations
        control_indices = np.where(control_active)[0]
        for i in control_indices:
            self.u[i] = self.Hs[i] - np.dot(self.Kx, self.x[i]) - np.dot(self.Ky, self.E[i])
        
        self.u = np.clip(self.u, self.w_min, self.w_max)
        
        # Vectorized rotor assignment
        self.rotors_vel[control_active, 0] = self.u[control_active, 1, 0]  # NW
        self.rotors_vel[control_active, 1] = self.u[control_active, 0, 0]  # NE
        self.rotors_vel[control_active, 2] = self.u[control_active, 3, 0]  # SW
        self.rotors_vel[control_active, 3] = self.u[control_active, 2, 0]  # SE
    ...
    def inform_operator(self, id, state, time, pos, flightplan, tracked_info, is_request_completed=False):
        ...
        payload = {
            "is_request": True, "sender": "uav", "id": id, "state": state,
            "time": time, "pos": serialized_pos, "flightplan": serialized_fp,
