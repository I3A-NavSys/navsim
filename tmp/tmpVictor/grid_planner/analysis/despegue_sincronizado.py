...
time_step = 0.01
is_3d = False
colors = ["cornflowerblue", "darkorange", "mediumseagreen"]
...
def get_exported_data():
    ...
    for i in range(3):
        fp_path = project_root_path + "/sims/exported_data" + f"/UAV{i}_flightplan.csv"
        tracked_info_path = project_root_path + "/sims/exported_data" + f"/UAV{i}_tracked_info.csv"
...
def build_plot(pos_fig_name, sep_fig_name):
    ...
    z_pos_time_plot.set_xlabel("Tiempo [s]")

    dist_sep_plot.set_xlabel("Tiempo [s]")
    dist_sep_plot.set_ylabel("Distancia [m]")
    
    # Set title
    xyz_pos_plot.set_title("Posición 3D")
    x_pos_time_plot.set_title("Pos vs Tiempo")
    
    dist_sep_plot.set_title("Distancia de separación")
...
def add_trace(tr, label, pos_fig_name, color):
    ...
    else:       xyz_pos_plot.plot(tr_x, tr_z, linewidth=2, label=label, color=color)
...
def hightlight_wps(tr, pos_fig_name):
    ...
    for i, time in enumerate(tr_t):
        ...
        if are_last_wps and truncated_time - last_wp_time == 10:
            ...
            else:           xyz_pos_plot.scatter(tr_x[i], tr_z[i], marker="o", color="blue", s=dot_size, zorder=3)
...
def add_distance_separation(dist_sep, tr1, tr2, times, label, pos_fig_name, sep_fig_name, color):
    ...
    if is_3d:
        ...
        xyz_pos_plot.plot([tr1_x, tr2_x], [tr1_z, tr2_z], color="black", linestyle="--", zorder=3)
