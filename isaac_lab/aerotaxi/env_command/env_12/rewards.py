...
def rew_lin_vel_diff(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    vel_command = env.command_manager.get_term("vel_command").target_vel
    
    error = torch.norm(lin_vel_b - vel_command, dim=1)
    return torch.exp(-error)

def rew_action_rate(env: ManagerBasedRLEnv):
...
#     asset = env.scene["aerotaxi"]
#     lin_vel_b = asset.data.root_com_lin_vel_b
#     vel_command = env.command_manager.get_command("vel_command").target_vel
...
#     yaw_rate_command = env.command_manager.get_command("vel_command").target_yaw
