...
class UAVcommandTerm(CommandTerm):
    ...
    def __init__(self, cfg: UAVcommandTermCfg, env: ManagerBasedRLEnv):
        ...
        # self.lookahead_time = torch.rand(self.num_envs, device=self.device) * 0.5 + 0.4
        self.lookahead_time = torch.zeros(self.num_envs, device=self.device)
        # tiempos de los que coger los puntos futuros: 0.5s y 1s
        # self.lookahead_times = torch.tensor([0.5, 1.0], device=self.device)

        # # pasos equivalentes
        # self.lookahead_steps = (self.lookahead_times / self.dt).long()

        # # tamaño máximo de cola
        # self.max_future_steps = int(self.lookahead_steps.max().item()) + 1

        # # buffer de posiciones futuras: future_target_pos[env_i, t_i, xyz]
        # self.future_target_pos = torch.zeros(self.num_envs,self.max_future_steps,3,device=self.device)
        # self.target_vel_goal = torch.zeros(self.num_envs, 3, device=self.device)
    ...
    def _resample_command(self, env_ids: torch.Tensor):
        ...
        # vel = self.target_vel[env_ids].clone()
        # yaw_rate = self.target_yaw[env_ids].clone()

        # for i in range(self.max_future_steps):

        #     # guardar posición
        #     self.future_target_pos[env_ids, i] = pos

        #     # rotación de la velocidad (misma lógica que update_command)
        #     cos_theta = torch.cos(yaw_rate * self.dt)
        #     sin_theta = torch.sin(yaw_rate * self.dt)

        #     vx = vel[:,0].clone()
        #     vy = vel[:,1].clone()

        #     vel[:,0] = vx * cos_theta - vy * sin_theta
        #     vel[:,1] = vx * sin_theta + vy * cos_theta
    ...
    def _update_command(self):
        ...
        # # Variación de pvelocidad dinámica x e y
        # if torch.rand(1) < 0.01:
        #     noise = torch.randn_like(self.target_vel[:, :2]) * 0.5
        #     self.target_vel[:, :2] += noise
        # self.target_vel_goal[change_z, 2] = (torch.rand(change_z.sum(), device=self.device) - 0.5) * 1.6

        # limitar aceleración
        # max_acc = 1.5  # m/s^2

        # vel_error = self.target_vel_goal - self.target_vel

        # acc = torch.clamp(vel_error, -max_acc * self.dt, max_acc * self.dt)
        ...
        # vel_xy = self.target_vel[:, :2]

        # desired_yaw = torch.atan2(vel_xy[:, 1], vel_xy[:, 0])

        # current_yaw = math_utils.euler_xyz_from_quat(
        #     self._asset.data.root_com_quat_w
        # )[2]
        ...
        # self._command[:, 3] = yaw_error

        # posiciones futuras
        # desplazar cola
        # self.future_target_pos[:, :-1] = self.future_target_pos[:, 1:].clone()

        # # añadir nuevo punto al final
        # self.future_target_pos[:, -1] = self.target_pos

        # # Comando para la red (Posición relativa en Body Frame)
        # uav_pos_local = self._asset.data.root_com_pos_w[:, :3] - self._env.scene.env_origins[:, :3]
        # # rel_pos_w = self.target_pos - uav_pos_local
        # pursuit_step = self.lookahead_steps[0]

        # target_pursuit = self.future_target_pos[:, pursuit_step]

        # rel_pos_w = target_pursuit - uav_pos_local
