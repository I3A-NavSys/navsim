...
# def rew_z_ang_vel_diff(env: ManagerBasedRLEnv) -> torch.Tensor:
#     obs = env.obs_buf
#     lin_vel = obs["policy"][:, 8]
#     vel_command = obs["policy"][:, 15]
#     rewards = torch.zeros(env.num_envs, device=env.device)
#     std=torch.tensor(0.5, device=env.device)

#     diff = lin_vel - vel_command
#     error = torch.square(diff)
#     rewards[:] = torch.exp(-error / std**2)
...
def pen_pos_diff(env: ManagerBasedRLEnv) -> torch.Tensor:
    obs = env.obs_buf
    origin_pos = env.scene.env_origins
...
def pen_z_lin_vel_diff(env: ManagerBasedRLEnv) -> torch.Tensor:
    obs = env.obs_buf
    lin_vel = obs["policy"][:, 5].abs()

    # diff = (lin_vel - vel_command).abs()
    # diff = torch.square(lin_vel) # must be 0 in rotation environment
...
def pen_xy_ang_vel_diff(env: ManagerBasedRLEnv) -> torch.Tensor:
    obs = env.obs_buf
    ang_vel = obs["policy"][:, 6:8]

    # diff = (lin_vel - vel_command).abs()
    # diff = torch.sum(torch.square(ang_vel), dim=1) # must be 0 in rotation environment
