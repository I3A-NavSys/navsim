...
def alt_routes():
    ...
    for alt_route in range(alternative_routes_amount*2):
        ...
        if route is not None:
            ...
            line = ax.plot(X, Y, zorder=3, label=f"Route {alt_route}: {length} - {round(e_time, 3)}s - {explored_nodes}")
...
def only_cost():
    for c in range(routes_amount):
        ...
        if route is not None:
            ...
            print(f"Route could not be found in time slot {takeoff_nodes_list[c][0].s}")
            print("--------------")
            print(f"Time: {e_time}")
            print(f"Explored nodes: {explored_nodes}")
            print("--------------")
