...
class NavsimCore(omni.ext.IExt):
    ...
    def on_startup(self, ext_id):
        ...
        logger = logging.getLogger("NavsimCore")
        ...
        dst = dst.as_posix() + "/app"
        
        # Get destination file path
        src = Path(os.path.expandvars(r"%APPDATA%\..\\Local\\ov\\pkg\\isaac-sim-4.1.0"))

        # Create the symlink if it doesn't already exist
        if src.exists():
            try:
                os.symlink(src.as_posix(), dst, target_is_directory=True)
                # print(f"Enlace simbólico creado: {src} -> {dst}")
                logger.info((f"Enlace simbólico creado: {src} -> {dst}"))
            except OSError as e:
                # print(f"Error al crear el enlace simbólico: {e}")
                logger.info(f"Error al crear el enlace simbólico: {e}")
        else:
            # print(f"El enlace simbólico ya existe: {dst}")
            logger.info(f"El enlace simbólico ya existe: {dst}")
