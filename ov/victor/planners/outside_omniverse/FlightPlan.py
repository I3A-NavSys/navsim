...
# INFORMATION AND FIGURES
    ...
    def PositionFigure(self, figName, timeStep):
        ...
        # Create the figure (I thinks it is the emergent window)
        fig = plt.figure(figName)

        # Create the desired graph
        graph = fig.add_subplot(111, projection = '3d')

        # We could set the limits of each axis by
        # graph.set_xlim([0,10])

        # We can set the names of the axis
        ...
        # Add to the graph the desired points
        tr = self.Trace(timeStep)
        graph.plot(tr[:, 1], tr[:, 2], '-', linewidth=2, color=color)
