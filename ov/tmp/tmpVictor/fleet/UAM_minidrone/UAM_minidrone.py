...
# Event handlers
    ...
    def on_play(self):
        ...
        # Crear waypoints
        # waypoint1 = Waypoint(label='WP1', t= 0,  pos=[0.0,0.0,1.0])
        # waypoint2 = Waypoint(label='WP2', t=10,  pos=[10.0,0.0,1.0])
        # # Crear plan de vuelo
        # self.fp = FlightPlan([waypoint1, waypoint2])
        # self.fp.radius = 2
        # self.fp.SetV0000()
        # self.fp.Print()
        # self.fp.RescheduleAt(2)
    ...
    def eventFn_FlightPlan(self, fp):
        ...
        self.fp.Postpone(self.current_time + 1)
        self.command.Hover()
    ...
    def navigation(self):
        ...
        if self.fp is not None:
            ...
            vel = np.array(self.linear_vel)
            current_status = Waypoint("status", self.current_time, pos, vel)

            self.command = self.fp.Navigate(self.prim.GetName(), self.rot, self.yaw, current_status, 5)
            ...
            if self.command is not None:
                if self.command.duration != 0:
                    ...
                    plt.close(plt.gcf())
                    self.fp.PositionFigure("FP1: POSITION", 0.01, self.xPos_track, self.yPos_track, self.zPos_track, self.time_track)
                    self.fp.VelocityFigure("FP1: VELOCITY", 0.01, np.array(self.xVel_track), np.array(self.yVel_track), np.array(self.zVel_track), self.time_track)
                    self.fp = None
