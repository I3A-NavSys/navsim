#!/usr/bin/env python3
"""
Correctness & Performance test suite for the OPTIMISED FlightPlan / Waypoint.

Targets:  flight_plan_new.py  +  waypoint_new.py

Run from the repository root:
    python3 uspace/flight_plan/test_flight_plan.py

Sections
--------
1. Helper builders
2. Unit / correctness tests  (unittest.TestCase)
   - Includes TestOptimizedBehavior: checks specific to the optimised code
   - Includes TestBugFixes: verifies the two bugs found in the original code
     are now FIXED (these tests are expected to PASS).
3. Performance benchmarks    (time.perf_counter)
4. Complexity notes & summary
"""
...
class TestBugFixes(unittest.TestCase):
    """
    Verifies that bugs found in the original code are FIXED in the optimised
    version.  All tests here are expected to PASS.
    """

    def test_fix_interpolation_crakle_attribute(self):
        """
        FIX: waypoint_new.py Waypoint.interpolation() now correctly writes
        'wp2.crakle = c2'.  With __slots__ the wrong name 'crkl' cannot even
        be set dynamically, and the correct attribute is always updated.
        """
        crakle_init = np.array([0.001, 0.0, 0.0])
        wp = Waypoint(
            t=0, pos=[0, 0, 0], vel=[5, 0, 0],
            acel=[0, 0, 0], jerk=[0.1, 0, 0],
            snap=[0.01, 0, 0], crakle=crakle_init.tolist()
        )
        result = wp.interpolation(5.0)

        # The stale-attribute must not exist (slots prevent dynamic creation)
        self.assertFalse(
            hasattr(result, 'crkl'),
            "'crkl' spurious attribute still present — fix not applied."
        )
        # The correct attribute must exist and be propagated
        self.assertTrue(
            hasattr(result, 'crakle'),
            "'crakle' attribute missing on interpolated Waypoint."
        )
        # crakle == c1 (constant across 5th-order integration)
        np.testing.assert_array_almost_equal(
            result.crakle, crakle_init, decimal=10,
            err_msg="crakle value not propagated correctly by interpolation()."
        )

    def test_fix_smooth_waypoint_speed_raises_runtime_error(self):
        """
        FIX: flight_plan_new.py smooth_waypoint_speed() error message no longer
        references the undefined 'label' variable.  Passing an invalid index
        (0 or last) must raise RuntimeError, not NameError.
        """
        fp = build_simple_fp()
        fp.set_uniform_velocity()

        for bad_wp in [0, len(fp.waypoints) - 1]:
            with self.assertRaises(RuntimeError,
                    msg=f"Expected RuntimeError for wp={bad_wp}, got something else"):
                fp.smooth_waypoint_speed(bad_wp, angVel=0.5)

    def test_fix_compare_to_non_overlapping_plans(self):
        """
        FIX: compare_to() now returns ([], np.array([])) when the two plans
        do not overlap in time instead of crashing with IndexError.
        """
        fp1 = build_simple_fp()   # t = 0..20
        fp1.connect_waypoints()
        fp2 = build_simple_fp()   # t = 100..120
...
class TestOptimizedBehavior(unittest.TestCase):
    """Tests specific to the optimised flight_plan_new / waypoint_new."""

    # ------------------------------------------------------------------
    # Waypoint.__slots__
    # ------------------------------------------------------------------

    def test_waypoint_has_slots(self):
        self.assertTrue(hasattr(Waypoint, '__slots__'),
                        "Waypoint must define __slots__ for memory efficiency.")

    def test_waypoint_no_dict(self):
        wp = Waypoint()
        self.assertFalse(hasattr(wp, '__dict__'),
                         "Waypoint instance must not have __dict__ (slots enabled).")

    def test_waypoint_slots_prevent_new_attributes(self):
        wp = Waypoint()
        with self.assertRaises(AttributeError):
            wp.nonexistent_field = 42

    def test_waypoint_deepcopy_works_with_slots(self):
        import copy
        wp = Waypoint(label="X", t=5.0, pos=[1, 2, 3], vel=[4, 5, 6],
                      acel=[0.1, 0.2, 0.3], jerk=[0.01, 0.02, 0.03])
        wp2 = copy.deepcopy(wp)
        np.testing.assert_array_equal(wp2.pos, wp.pos)
        self.assertIsNot(wp2.pos, wp.pos)   # deep — distinct arrays

    # ------------------------------------------------------------------
    # Bisect-based get_target_index_from_time
    # ------------------------------------------------------------------

    def test_bisect_exact_match_at_every_waypoint(self):
        """bisect_left: t == wp.t must return the index of that waypoint."""
        fp = build_large_fp(10)
        for i, wp in enumerate(fp.waypoints):
            idx = fp.get_target_index_from_time(wp.t)
            self.assertEqual(idx, i,
                msg=f"bisect index wrong at wp {i}, t={wp.t}: got {idx}")

    def test_bisect_between_waypoints(self):
        """t strictly between two WPs must return the index of the next WP."""
        fp = build_large_fp(5)   # times: 0, 10, 20, 30, 40
        self.assertEqual(fp.get_target_index_from_time(5),  1)
        self.assertEqual(fp.get_target_index_from_time(15), 2)
        self.assertEqual(fp.get_target_index_from_time(25), 3)

    def test_bisect_before_first_waypoint(self):
        fp = build_large_fp(5)
        self.assertEqual(fp.get_target_index_from_time(-999), 0)

    def test_bisect_after_last_waypoint(self):
        fp = build_large_fp(5)
        self.assertEqual(fp.get_target_index_from_time(9999), len(fp.waypoints))

    def test_bisect_single_waypoint(self):
        fp = FlightPlan()
        fp.set_waypoint(label="A", time=5)
        self.assertEqual(fp.get_target_index_from_time(5),  0)
        self.assertEqual(fp.get_target_index_from_time(0),  0)
        self.assertEqual(fp.get_target_index_from_time(10), 1)

    # ------------------------------------------------------------------
    # Vectorised trace
    # ------------------------------------------------------------------

    def test_trace_matches_status_at_time_pointwise(self):
        """Vectorised trace must agree with status_at_time on sampled points."""
        fp = build_connected_fp(n_segments=4)
        step = 1.0
        tr = fp.trace(step)
        for row in tr:
            t = row[0]
            s = fp.status_at_time(t)
            np.testing.assert_array_almost_equal(
                row[1:4], s.pos, decimal=4,
                err_msg=f"trace pos != status_at_time pos at t={t:.2f}"
            )
            np.testing.assert_array_almost_equal(
                row[4:7], s.vel, decimal=4,
                err_msg=f"trace vel != status_at_time vel at t={t:.2f}"
            )

    def test_trace_returns_numpy_array(self):
        fp = build_simple_fp()
        fp.connect_waypoints()
        tr = fp.trace(1.0)
        self.assertIsInstance(tr, np.ndarray)

    # ------------------------------------------------------------------
    # Vectorised compare_to
    # ------------------------------------------------------------------

    def test_compare_to_returns_numpy_distances(self):
        fp1 = build_large_fp(5)
        fp2 = build_large_fp(5)
        for wp in fp2.waypoints:
            wp.pos[1] += 10
    ...
    def test_compare_to_non_overlapping_returns_empty(self):
        fp1 = build_simple_fp()   # 0..20
        fp1.connect_waypoints()
        fp2 = build_simple_fp()   # 100..120
        fp2.reschedule_at(100)
        fp2.connect_waypoints()
        distances, times = fp1.compare_to(fp2, 1.0)
        self.assertEqual(len(distances), 0)
...
def run_benchmarks(verbose: bool = True) -> list:
    ...
    #    set_waypoint: O(log n) index (bisect) + O(n) list.insert → O(n) each
    #    Building n waypoints: O(n²) overall due to list.insert shifting.
    # ------------------------------------------------------------------
    if verbose:
        print("\n── Build / insertion ─────────────────────────────────────────")
    for n in [10, 50, 100, 500]:
        ...
        record(f"build_fp (n={n:4d} waypoints)", t_us,
               note="O(n) per insert — O(n²) total (list.insert unavoidable)")

    # ------------------------------------------------------------------
    # 2. Index lookup — get_target_index_from_time  [O(log n) bisect]
    # ------------------------------------------------------------------
    if verbose:
        print("\n── get_target_index_from_time  [O(log n) bisect] ─────────────")
    ...
    # ------------------------------------------------------------------
    # 3. status_at_time — O(log n) bisect + polynomial interpolation
    # ------------------------------------------------------------------
    if verbose:
        print("\n── status_at_time  [O(log n) bisect + interpolation] ─────────")
    for n in [10, 100, 500]:
        fp = build_large_fp(n)
...
if __name__ == "__main__":
    ...
    if n_fail == 0 and n_error == 0:
        print("  All tests PASSED — optimised version is correct.")
