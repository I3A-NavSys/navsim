...
if "%1"=="" (
    echo Usage: %0 ^<configuration^> ^<isaac_sim_path^>
    echo.
    echo Available configurations:
    echo   navsim-only          - Install only NAVSIM project
    echo   navsim-grid-planner  - Install NAVSIM + Grid Planner
    echo   navsim-full         - Install NAVSIM + All Extensions
    echo.
    echo Example: %0 navsim-grid-planner C:\isaacsim
    pause
    exit /b 1
)
...
set COMPOSE_FILE=docker-compose.%CONFIG%.yml
